...
class BinarySearchTree:
    ...
    def __sorted_list_to_bst(self, nums, left, right,called_function:str = ''):
        if left > right:
            return None
        middle = (left + right) // 2
        current = Node(nums[middle])

        current.left = self.__sorted_list_to_bst(nums, left, middle - 1, 'left')
        current.right = self.__sorted_list_to_bst(nums, middle + 1, right, 'right')
        return current
